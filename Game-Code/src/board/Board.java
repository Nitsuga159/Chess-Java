...
public class Board extends JLayeredPane {
	...
	private void drawBoard() {
		...
		for(int row = 0; row < LENGTH_BOARD; row++) {
			...
			for(int column = 0; column < LENGTH_BOARD; column++) {
				...
				Piece piece = getCellPiece(row, column);
				...
				if(piece != null)
					labelIcon.setIcon(
						MyWindow.getImageIcon(piece.getImageURL())
					);
	...
	private void initPieces() {
		...
		public void fill(UUID color, boolean inEnd) {
			...
			for(int column = 0; column < LENGTH_BOARD; column++)
				cells[secondRow][column].setPiece(new Pawn(this, color, inEnd, secondRow, column));
		}
	...
	public void setWinner(UUID winner) {
		this.winner= winner;
		add(
			new Winner(winner.equals(Piece.BLACK) ? "black" : "white", this), 
			0);
	}
	...
	public final static int SIZE = MyWindow.SCREEN.height > 1000 ? 800 : 600 ;
